...
def load_data_from_csv(fname: str, input_root: str) -> List[Tuple[str, str | None]]:
    ...
    for idx, row in data.iterrows():
        # 使用絕對路徑
        im_path = row["Input"] if os.path.isabs(row["Input"]) else os.path.join(input_root, row["Input"])
        mask_path = None
        if label_present:
            mask_path = row["Label"] if os.path.isabs(row["Label"]) else os.path.join(input_root, row["Label"])
